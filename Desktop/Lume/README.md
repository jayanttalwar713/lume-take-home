...
**Evaluation results: 36/36 scenarios passed (100%)**

---

## AI Tool Usage

Claude (Claude Code) was used extensively throughout this project. My role was to drive the design — deciding on the architecture, evaluating tradeoffs, and determining what the right approach was at each step. Claude handled the majority of implementation once the design direction was set.

Specifically, Claude was used to:
- **Write the code** — agent orchestration, RAG pipeline, output filtering, decision logging, evaluation harness
- **Expand the policy document** — the spec provides a 6-section seed and instructs candidates to use an LLM to expand it to 15–30 pages; Claude generated the full 20-section policy
- **Generate the extended test suite** — the 15 additional scenarios beyond the 21 provided were generated by prompting Claude with the policy and asking it to cover edge cases not addressed in the provided set
- **Discuss design choices** — key architectural decisions (section-aware vs. naive chunking, hard Python guards vs. LLM-only enforcement, output filtering at tool boundary vs. at response generation) were discussed and evaluated before implementation

The design choices — what to build, why, and where the tradeoffs lie — are my own and I can speak to all of them.
...
- **Grey-tier scoring**: Replace LLM judgment on "clarify vs escalate" with a deterministic risk score based on data sensitivity + drive classification, so behaviour is consistent across runs.
